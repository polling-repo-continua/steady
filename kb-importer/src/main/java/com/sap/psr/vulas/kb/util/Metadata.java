...
public class Metadata {
  ...
  public static Commit getCommitMetadata(String commitDir) throws JsonSyntaxException, IOException {
    ...
    if (!FileUtil.isAccessibleFile(filePath)) {
      log.error("The commit folder {} or the meta file is missing {} in commit folder", commitDir,
          filePath);
      return null;
    }
  ...
  public static Vulnerability getVulnerabilityMetadata(String rootDir)
    ...
    if (!FileUtil.isAccessibleFile(filePath)) {
      throw new IllegalArgumentException("The root folder " + rootDir
          + "  or the meta file in root directory is missing " + filePath);
    }
